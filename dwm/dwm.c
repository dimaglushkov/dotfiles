...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
			continue;
		/* manually updated: use pango fonts for tags*/
		w = TEXTWM(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i, True);
		x += w;
	}
...
restack(Monitor *m)
{
	...
	XSync(dpy, False);
	while (XCheckMaskEvent(dpy, EnterWindowMask, &ev));
	// remove to get rid of warping on every new window
...
centeredmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx, m->wy + my, mw - 2*bw, h - 2*bw, 0, 0);
		my += HEIGHT(c);
	} else {
		/* stack clients are stacked vertically */
		if ((i - m->nmaster) % 2 ) {
			h = (m->wh - ety) / ( (1 + n - i) / 2);
			resize(c, m->wx, m->wy + ety, tw - 2*bw, h - 2*bw, 0, 0);
			ety += HEIGHT(c);
		} else {
			h = (m->wh - oty) / ((1 + n - i) / 2);
			resize(c, m->wx + mx + mw, m->wy + oty, tw - 2*bw, h - 2*bw, 0, 0);
			oty += HEIGHT(c);
		}
	}
}
...
centeredfloatingmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx, m->wy + my, w - (2*c->bw), mh - (2*c->bw), 0, 1);
		mx += WIDTH(c);
	} else {
		/* stack clients are stacked horizontally */
		w = (m->ww - tx) / (n - i);
		resize(c, m->wx + tx, m->wy, w - (2*c->bw), m->wh - (2*c->bw), 0, 1);
		tx += WIDTH(c);
	}
}
